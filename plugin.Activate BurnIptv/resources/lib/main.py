...
url = "http://playweb.x10.mx/phpbb/ucp.php?mode=login&redirect=.%2Findex.php"
...
def LOGIN(username,password,hidesuccess):
	...
	if logged_in == True:
		...
		Bienve1 = dialogo.ok('[COLOR red]Bienvenidos a [/COLOR]***[COLOR blue]Activate BurnIptv  [/COLOR]***','[COLOR red]     El plugin Activate BurnIptv   Activador[/COLOR]','Esperamos Disfruteis de este plugin hecho para ustedes,  no se hace RESPONSABLE del uso que les den a este plugin esto es solo, con fines de investigacion.','[COLOR blue]----------------------[COLOR blue]http://playweb.x10.mx/[/COLOR]-----------------------[/COLOR] ')
		import activador
	...
	if logged_in == False:
		Notificaciones('[COLOR blue]Visite http://playweb.x10.mx/[/COLOR]','El Usuario y Contraseña no son Correcto Introduzcalo de nuevo','5000',notierror)
		
	        Bienve1 = dialogo.ok('Bienvenidos a [COLOR blue]Activate BurnIptv ','###### [COLOR red]El Usuario y Contraseña no son Correcto[/COLOR]######','########         Vuelva a intentarlo de nuevo       ########','###### O Visite  [COLOR blue]  http://playweb.x10.mx/ [/COLOR] #######')
