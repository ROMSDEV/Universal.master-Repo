...
class ScraperResult():
	...
	x265_color = ADDON.get_setting('custom_color_x265') if ADDON.get_setting('custom_color_x265') != '' else 'chocolate'
	...
	def __init__(self, debrid_hosts, service, hostname, url, text=None):
		...
		#if re.search('(\.x265)|(\.hevc\.)|(', self.text, re.IGNORECASE):
		if re.search('([-_\s\.]?(x265)|(hevc)[-_\s\.])', self.text, re.IGNORECASE):
			self.x265 = 'x265'
	...
	def format(self):
		...
		for foo in ['size', 'bitrate', 'extension', 'x265', 'quality']:
			self.ck(foo)
	...
	def search_tvshows(self, showname, season, episode, year=None, imdb_id=None, tmdb_id=None, tvdb_id=None, return_sources=False):
		...
		for index in range(0, self.enabled_scrapers):
			...
			if self.cache_results and self.get_scraper_by_index(index).is_cachable:
				...
				cached = [pickle.loads(str(r[0])) for r in results]
			else:
				cached = False
			if cached:
				number = len(cached)
