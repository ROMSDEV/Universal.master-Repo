...
class TransmogrifierAPI:
	...
	def get_cached_file(self, title, season=None, episode=None, year=None):
		...
		if season is not None:
			for extension in ['avi', 'mkv', 'mov', 'mp4', 'flv']:
				path = vfs.join(self.save_directory, "TV Shows/%s.S%sE%s.%s" % (title, str(season).zfill(2), str(episode).zfill(2), extension))
				if vfs.exists(path):
					return path
	...
	def _call(self, method, data=None):
		...
		try:
			...
			ADDON.log("TransmogrifierAPI Error: %s" % e, LOG_LEVEL.VERBOSE)
