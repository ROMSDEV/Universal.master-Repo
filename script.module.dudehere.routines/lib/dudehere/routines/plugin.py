...
class Plugin():
	...
	def dialog_file_browser(self, title, mask=''):
	...
	def get_hash_id(self, imdb_id='', season='', episode=''):
		import hashlib
	...
	def play_stream(self, url,  metadata={"cover_url": "", "title": ""}, title=None):
		...
		if ADDON.get_setting('enable_resume') == 'true':
			if 'season' in metadata:
				hash_id = self.get_hash_id(metadata['imdb_id'], metadata['season'], metadata['episode'])
			else:
				hash_id = self.get_hash_id(metadata['imdb_id'], metadata['slug'])
